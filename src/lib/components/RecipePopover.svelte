<script lang="ts">
	...
	let { itemId, children }: Props = $props();
	...
	const allRecipes = $derived(findRecipesForItem(itemId));

	// Check if a recipe is a "downgrade" (output tier < input tier)
	function isDowngradeRecipe(recipe: typeof allRecipes[0]): boolean {
		const outputItem = getItemById(itemId);
		if (!outputItem || outputItem.tier === -1) return false;
			...
			<div class="flex items-center justify-between">
				<div>
					{#if recipe.craftingStationName}
						<span class="text-blue-400">{recipe.craftingStationName}</span>
						{#if recipe.craftingStationTier}
							<span class="text-gray-500"> T{recipe.craftingStationTier}</span>
						{/if}
					{:else}
						<span class="text-gray-500">Crafting Recipe</span>
					{/if}
					{#if recipe.outputQuantity > 1}
						<span class="text-gray-500 ml-1">(×{recipe.outputQuantity})</span>
					{/if}
				</div>
