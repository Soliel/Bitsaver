...
<aside class="flex h-full flex-col bg-gray-900/80 backdrop-blur-xl border-r border-white/10 text-white transition-all duration-200 {collapsed ? 'w-16' : 'w-56'}">
	...
	<div class="border-t border-white/10 p-4">
		<a
			...
			class="flex items-center gap-2 text-gray-400 hover:text-white transition-colors {collapsed ? 'justify-center' : ''}"
		>
		...
		{#if !collapsed}
			<p class="mt-2 text-xs text-gray-500">Powered by Bitjita API</p>
		{/if}
	</div>
</aside>
